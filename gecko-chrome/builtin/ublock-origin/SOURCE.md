...
The XPI is installed only from Navis's application-owned `extensions/` directory. Navis does not unpack, patch, re-sign or independently update the extension code. A new extension version therefore requires an explicit Navis source change and application release. uBlock Origin's filter-list updater is separate and remains governed by its own user-visible controls.
